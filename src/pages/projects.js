...
const LargeScaleProject = ({ projectImg, projectType, projectTitle, projectDescription, gitHubURL, projectURL, disableGithubURL, disableProjectURL, disableProjectImageURL, disableProjectTitleURL }) => {
      ...
      {isMobile ? (
        <Tilt glareEnable={true} glareColor={`${mode === 'light' ? '#FFFFFF' : '#FFFFFF'}`} glareBorderRadius='1.5rem'>
          <article className='w-full flex relative items-center justify-between rounded-br-3xl rounded-3xl border-2 border-solid border-dark bg-light dark:border-light dark:bg-dark shadow-2xl p-12 xl:flex-col xl:p-8 xs:rounded-2xl xs:rounded-br-3xl xs:p-4'>
            ...
            <Link href={projectURL} onClick={disableProjectImageURL} target={"_blank"} className='w-1/2 cursor-pointer overflow-hidden rounded-lg xl:w-full'>
            ...
            <div className='w-1/2 flex flex-col items-start justify-between pl-6 xl:w-full xl:pl-0 xl:pt-6'>
              ...
              <Link href={projectURL} onClick={disableProjectTitleURL} target={"_blank"} className='hover:underline underline-offset-4 text-dark dark:text-light'>
              ...
              <div className='w-full mt-2 flex items-center justify-between'>
                <Link href={projectURL} onClick={disableProjectURL} target={"_blank"}
                ...
                <MotionLink href={gitHubURL} onClick={disableGithubURL} target={"_blank"} whileHover={{ y: -4 }} whileTap={{ scale: 0.8 }} className='w-10 ml-4 text-dark dark:text-light md:w-8'> <GithubIcon /> </MotionLink>
              </div>
        ...
        <Tilt glareEnable={true} glareColor={`${mode === 'light' ? '#FFFFFF' : '#FFFFFF'}`} glareBorderRadius='1.5rem' tiltMaxAngleY='8' tiltMaxAngleX='8' >
          <article className='w-full flex relative items-center justify-between rounded-br-3xl rounded-3xl border-2 border-solid border-dark bg-light dark:border-light dark:bg-dark shadow-2xl p-12 xl:flex-col xl:p-8 xs:rounded-2xl xs:rounded-br-3xl xs:p-4'>
            ...
            <div className='w-1/2 flex flex-col items-start justify-between pl-6 xl:w-full xl:pl-0 xl:pt-6'>
              ...
              <div className='w-full mt-2 flex items-center justify-between'>
                <Link href={projectURL} onClick={disableProjectURL} target={"_blank"}
                  className='rounded-lg p-2 px-6 bg-dark text-light dark:bg-light dark:text-dark text-lg font-semibold hover:bg-light dark:hover:bg-dark hover:text-dark dark:hover:text-light border-2 border-solid border-transparent hover:border-dark dark:hover:border-light duration-700 sm:px-4 sm:text-base xs:text-sm xxs:px-3 xxxs:px-2 xxxs:text-xs'>Visit Project</Link>
                <MotionLink href={gitHubURL} onClick={disableGithubURL} target={"_blank"} whileHover={{ y: -5 }} whileTap={{ scale: 0.8 }} className='w-10 ml-4 text-dark dark:text-light md:w-8'> <GithubIcon /> </MotionLink>
              </div>
...
const SmallScaleProject = ({ projectImg, projectType, projectTitle, projectDescription, gitHubURL, projectURL, disableGithubURL, disableProjectURL, disableProjectImageURL, disableProjectTitleURL }) => {
  ...
  return (
    <Tilt glareEnable={true} glareColor={`${mode === 'light' ? '#FFFFFF' : '#FFFFFF'}`} glareBorderRadius='1rem'>
      <article className='w-full h-auto flex flex-col items-center justify-center rounded-2xl border-2 border-solid border-dark bg-light dark:border-light dark:bg-dark p-6 relative xs:p-4'>
        ...
        <Link href={projectURL} onClick={disableProjectImageURL} target={"_blank"} className='w-full cursor-pointer overflow-hidden rounded-lg'>
        ...
        <div className='w-full flex flex-col items-start justify-between mt-4'>
          ...
          <div className='w-full mt-2 flex items-center justify-between'>
            ...
            <MotionLink href={gitHubURL} onClick={disableGithubURL} target={"_blank"} whileHover={{ y: -5 }} whileTap={{ scale: 0.8 }} className='w-10 text-dark dark:text-light md:w-8'> <GithubIcon /> </MotionLink>
          </div>
  ...
  return (
    <>
      <Head>
        ...
        <meta property="og:description" content="Projects | Ryan Abeysinghe's Personal Portfolio using Next.js, JavaScript, Tailwind CSS, HTML, Nodemailer and Vercel utilizing JavaScript React Framework" />
